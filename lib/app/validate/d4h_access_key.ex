defmodule App.Validate.D4HAccessKey do
  ...
  def call(changeset, access_key_field, api_host_field, d4h_team_field) do
    changeset
    |> validate_length(access_key_field, min: 5, max: 100)
    |> validate_inclusion(api_host_field, Map.values(D4H.regions()))
    |> validate_d4h_api_auth(access_key_field, api_host_field, d4h_team_field)
  end

  defp validate_d4h_api_auth(%{valid?: false} = changeset, _, _, _), do: changeset

  defp validate_d4h_api_auth(changeset, access_key_field, api_host_field, d4h_team_field) do
    access_key = get_field(changeset, access_key_field)
    api_host = get_field(changeset, api_host_field)
    d4h = D4H.build_context(access_key: access_key, api_host: api_host)
    ...
    try do
      case D4H.fetch_team(d4h) do
        ...
        {:ok, d4h_team} ->
          put_change(changeset, d4h_team_field, d4h_team)
      end
    rescue
